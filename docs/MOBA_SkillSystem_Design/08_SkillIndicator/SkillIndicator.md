...
的指示器系统涉及 4650 行的 技能指示器控制 和 351 行的 指示器基类，包含大量特殊英雄定制逻辑（安琪拉多箭头、孙膑双圈等）。
...
│ · : 挂场景根 Transform, 含 Normal/Block/Grass 三套材质 │
│ · Roblox: Part 独立定位                                   │
├─────────────────────────────────────────────────────────┤
│ Fixed 层 (固定层)                                        │
...
```lua
-- 来源: EnableSkillCursor → CreateIndicatePrefab
-- 按下技能按钮时调用，创建三层 Part
...
```lua
-- 来源: DisableSkillCursor → UnInitIndicatePrefab
...
-- 通过 Layer 切换 (Actor/Particles ↔ Hide) 实现
...
```lua
-- 来源: LateUpdate(int nDelta) 第2645-2903行 中的 Lerp/Slerp 逻辑
...
原版不同技能类型用不同插值速度，Roblox 版统一用 `15` 作为起始值。实现者可根据手感需要对不同类型使用不同系数。
