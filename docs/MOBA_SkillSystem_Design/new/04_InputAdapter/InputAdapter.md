...
> **HOK 来源**: Unity `EventTrigger` (OnPointerDown/OnDrag/OnPointerUp)

---

## 设计说明

HOK 使用 Unity 的 `EventTrigger` 组件在 UI 元素上监听 Pointer 事件。Roblox 没有对等组件，需要用 `UserInputService` + `GuiButton.InputBegan` 组合实现。
