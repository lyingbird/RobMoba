...
> **HOK 来源**: `IndicatorHelper.cs` (1161行) + `SkillIndicatorBase.cs` (351行)  
...
HOK 的指示器渲染涉及：
- **IndicatorHelper** — 1161行工具类：缩放计算、显隐控制（Layer切换）、颜色设置、Prefab创建
- **SkillIndicatorBase** — 351行：三层资源创建/销毁/位置设置
...
| HOK | Roblox | 说明 |
|---|---|---|
| Layer 切换 Actor↔Hide | Transparency 0.5↔1 | HOK 通过修改 GameObject.layer 实现，摄像机只渲染 Actor 层 |
| SetActive(false) | Transparency = 1 | 推荐 Transparency 而非 part.Parent = nil，避免频繁创建/销毁 |

### HOK 缩放公式（来源: IndicatorHelper.GetIndicatorScaleLength）
...
```lua
-- 来源: HOK IndicatorHelper.CreatePrefab() + SkillIndicatorBase.CreateIndicatePrefab()
-- 根据资源类型创建对应形状的 Part
--
-- 参数:
...
```lua
-- 来源: HOK SkillIndicatorBase.UnInitIndicatePrefab()
-- HOK 使用对象池回收，Roblox 简化为直接销毁
-- 高级优化: 可改为对象池模式
...
-- 来源: HOK SkillIndicatorBase.SetEffectPrefabPosition()
...
```lua
-- 来源: HOK IndicatorHelper.ShowPrefab() / HidePrefab()
-- HOK 通过 Layer 切换 (Actor↔Hide)
...
```lua
-- 来源: HOK IndicatorHelper.SetEffColor() / SetMatTintColor()
-- 用于取消区域红色提示、延迟确认黄色等
...
HOK 使用 `CreatePrefab` 从对象池获取，`UnInitIndicatePrefab` 回收到池。高频创建/销毁 Part 会有性能开销，建议实现简单的对象池：
