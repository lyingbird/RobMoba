...
> **HOK 来源**: `CSkillButtonManager.cs`（10535行）— 精简为 Down/Drag/Up 分发 + 取消区域 UI 刷新

---

## 设计说明

HOK 的 `CSkillButtonManager` 是一个万行级巨类，包含按钮管理、指示器创建、取消 UI、蓄力、锁定框、多技能联动等。Roblox 版拆分后，此模块只保留：
...
--   cancelUI   { CancelFrame = Frame, CancelTips = ImageLabel }
--
-- 来源: HOK CSkillButtonManager 初始化阶段（分散在多个 Init 方法中）
...
-- _OnDrag: 拖动
-- ================================================================
-- 来源: HOK OnSkillButtonDrag() 第590-723行 中的 UI 刷新部分
...
-- ================================================================
-- 来源: HOK CSkillButtonManager.RefreshIndicatorCancelUI() 第6386-6429行
--
-- 逻辑:
--   在取消区域内:
