...
> **HOK 来源**: `ResSkillIndicatorCfgInfo`（48字节）、`ResSkillDynamicIndicatorCfgInfo`（128字节）、`ResSkillMultiIndicatorCfgInfo`（144字节）

---

## 设计说明

HOK 有 **4种指示器配置表**（基础/动态/多段/特殊动态），总计超过200个字段。Roblox 版合并为**单表单结构**，用字段开关控制三层（Guide/Effect/Fixed）的启用和参数。

### HOK 配置表精简映射

| HOK 配置表 | 字节数 | Roblox 处理 |
|---|---|---|
| `ResSkillIndicatorCfgInfo` | 48 | → IndicatorConfig 主结构 |
| `ResSkillDynamicIndicatorCfgInfo` | 128 | → 合并到主结构（动态尺寸用 `effectRadius` 运行时修改） |
| `ResSkillMultiIndicatorCfgInfo` | 144 (3段) | → 多段技能拆成多条配置，运行时切换 |
| `ResSkillSpecialDynamicIndicatorCfgInfo` | 88 (3段) | → 同上 |
...
-- └────────────────────────────────────────────────────────────┘
--
-- 缩放公式 (来源 HOK IndicatorHelper.GetIndicatorScaleLength):
