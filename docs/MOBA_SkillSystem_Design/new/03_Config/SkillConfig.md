...
> **HOK 来源**: `ResSkillCfgInfo`（技能基础配置）+ `ResDT_SkillFunc`（技能功能数据）

---

## 设计说明

HOK 的技能配置散布在多张 TDR 配置表中（`ResSkillCfgInfo`、`ResDT_SkillFunc`、`ResDT_Skill_Param` 等），字段超过100个。Roblox 版精简为**与技能释放交互直接相关**的字段，其余伤害/Buff 等字段由实现者根据自己的战斗系统补充。
