...
> **HOK 来源**: `SkillBtnController.OnSkillButtonUp()` 第308-588行  
...
```lua
-- 来源: HOK SkillBtnController.OnSkillButtonUp() 第308-588行
-- 精简说明:
--   HOK 原版还有: 蓄力 ForceUp、多手指冲突解决、方向锁定模式、延迟确认
...
function SC:OnButtonUp(slot, screenPos)
    ...
    -- ① 最后一次 Drag
    -- HOK 做法: 抬手时用最终位置再 SelectSkillTarget 一次
    ...
    -- ③ 取消判定
    -- 来源: HOK OnSkillButtonUp 第480-510行
    -- 原文逻辑:
...
1. **最后一次 Drag 在 Up 开头执行**: 这不是多余的调用。如果用户快速拖动并抬起，最后一个 Drag 事件的位置可能不是抬起位置。HOK 在 Up 开头补一次 Drag 确保最终位置准确。
