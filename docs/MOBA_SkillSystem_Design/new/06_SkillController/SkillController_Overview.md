...
> **HOK 来源**: `SkillBtnController.cs` (966行) + `SkillControlIndicator.cs` (4650行) 合并精简  
...
HOK 的技能控制分散在两个类中：
- `SkillBtnController` — UI 事件分发（OnButtonDown/Drag/Up）
- `SkillControlIndicator` — 指示器逻辑 + 目标选择 + 状态管理
