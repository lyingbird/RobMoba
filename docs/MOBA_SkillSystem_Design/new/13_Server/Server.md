...
> **HOK 来源**: `ActorControler.CmdUseSkill()`（校验大门）+ `BaseSkill.UseImpl()`（执行核心）
...
-- HOK 等价: ActorControler.CmdUseSkill()（校验大门）
...
-- 校验清单 (来源: HOK ActorControler.CmdUseSkill)
--
--   ① 技能配置是否存在
--   ② CD 是否就绪
--   ③ 蓝量是否足够
