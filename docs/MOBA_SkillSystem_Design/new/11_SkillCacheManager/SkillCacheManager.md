...
> **HOK 来源**: `SkillCache.h` (392行) + `SkillCache.cpp` (1125行) + `SkillInputCacheDuration.cpp` (272行)  
> **重要性**: ★★★★★ 这是影响操作手感的核心模块
...
function SCM.new(deps)
    ...
    -- ========== 缓存窗口 ==========
    -- HOK 中由 AGE 动画轨道 SkillInputCacheDuration 控制
    -- Roblox 简化为由技能释放事件触发
...
```lua
-- 来源: HOK SkillCache::PushCacheSkillList() 第333-382行
--
-- 两种模式:
...
```lua
-- 来源: HOK SkillCache::UseSkillCache() 第618-755行
-- 由 SkillController.OnSkillCastFinished() 在技能释放完毕后调用

function SCM:TryUseCache()
    -- ① 过期检查
    -- 来源: HOK CheckSkillCacheExpried() 第843-858行
    ...
    -- ② 普攻缓存优先
    -- HOK 逻辑: cacheCommonAttack && !IgnoreCommonAttack
...
-- 连续普攻窗口
-- ================================================================
-- 来源: HOK SkillCache::SetCacheContinuAttackCacheInfo() 第959-990行
...
-- 来源: HOK SetCacheContinueNormalAttackContext() 第408-435行
-- 在窗口期内按下普攻 → 标记连续普攻缓冲
...
-- 受控保护机制
-- ================================================================
-- 来源: HOK SkillCache::OnBuffChange() 第168-212行
--
-- 场景: 玩家正在连招，突然被眩晕
...
-- 缓存窗口控制
-- HOK 中由 AGE 动画轨道 SkillInputCacheDuration 的 Enter()/Leave() 触发
-- Roblox 中由技能释放动画事件触发
...
| 常量 | 值 | HOK 枚举 | 含义 |
|---|---|---|---|
| `SKILL_CACHE_MAX_COUNT` | 1 | RES_GLOBAL_CONF_TYPE #1197 | 技能缓冲队列上限 |
| `SKILL_CACHE_EXPIRED_TIME` | 2.0s | RES_GLOBAL_CONF_TYPE #1198 | 缓存过期时间 |
| `CONTINUE_ATTACK_WINDOW_BEGIN_PCT` | 70% | RES_GAMECORE_CONF_TYPE #3 | 连续普攻窗口开始 |
| `CONTINUE_PURSUE_WINDOW_BEGIN_PCT` | 50% | RES_GAMECORE_CONF_TYPE #14 | 追击窗口开始 |
| `CONTROL_PROTECT_DURATION` | 0.5s | SkillInputCacheDuration | 受控保护窗口 |
...
| 场景 | 操作 | HOK 来源 |
|---|---|---|
| 死亡 | `Clear()` | `SkillCache.cpp` |
| 切 AutoAI | `Clear()` | `SkillCache.cpp` |
| 受控解除后过期 | `Clear()` | `OnBuffChange()` |
| 新技能入队时 | `ClearCommonAttackCache()` | `PushCacheSkillList()` |
| 缓存过期 | `Clear()` | `CheckSkillCacheExpried()` |
