...
的全局配置分散在多个系统中（全局配置表、GameCore配置表、代码硬编码）。Roblox 版集中到一个文件，每个常量标注 来源，方便调参。
